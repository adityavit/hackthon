...
public class ParselyExtractor {
	...
	private static String[] publishersList = {"bloomberg","foxnews"};
	...
	public HashMap<String,ArrayList<ParselyModel>> fetchParselyData(String[] tokens)
			throws UnsupportedEncodingException {
		JsonElement jse = null;
		String queryString = URLEncoder.encode(concat(tokens), "UTF-8");
		for(int j=0;j<publishersList.length;j++){
		String publisherApi = parselyApi.replaceAll("###", publishersList[j]);
		String parselyUrl = publisherApi + queryString;
		System.out.println("queryString="+queryString+" url="+parselyUrl);
		try {
			...
			if (resHeaderElement.get("status").getAsString()
					.equalsIgnoreCase("0")) {
				JsonObject response = mainObj.get("response").getAsJsonObject();
					...
					if (docsArr != null) {
						...
						if (docsArr.size() != 0) {
							...
							for (int i = 0; i < docsArr.size() && i<5; i++) {
								parselyModelList.add(new ParselyModel(docsArr
										.get(i)));
							}
							parselyModelMap.put(publishersList[j], parselyModelList);
						}
					}
				}
			}
